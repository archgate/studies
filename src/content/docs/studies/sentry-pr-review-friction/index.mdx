...
4. **Automated reviewers find real bugs — but the same bug categories repeat across PRs.** Bot reviewers (`sentry[bot]`, `sentry-warden[bot]`, `cursor[bot]`) account for **23.7%** of substantive review comments and appear in **68.3%** of high-friction PRs. Reading the actual findings reveals **at least 10 recurring patterns** (missing `DoesNotExist` handlers, `.filter().first()` with unreachable except, direct dict access on API responses, option key mismatches, companion list misses, etc.) that should be promoted from expensive agentic review to cheap deterministic checks (Ruff/Semgrep rules, mypy strict mode, typed registries). One PR ([#111522](https://github.com/getsentry/sentry/pull/111522)) had the same pattern flagged **4 times in one review pass** — exactly the case where a single lint rule pays off forever.
